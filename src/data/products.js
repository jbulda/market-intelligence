export const products = [
    // --- CORE COMPONENTS ---
    {
        id: "CPU-9800X3D",
        name: "AMD Ryzen 7 9800X3D",
        category: "CPU",
        wattage: 120, // Default TDP
        price: 479.99,
        brand: "AMD",
        specs: ["Zen 5 8-Core", "96MB L3 V-Cache", "5.2 GHz Boost"]
    },
    {
        id: "GPU-9070XT-ICE",
        name: "GIGABYTE Radeon RX 9070 XT",
        category: "GPU",
        wattage: 304, // Rated TBP (Peak Draw)
        price: 599.99,
        brand: "GIGABYTE",
        specs: ["16GB GDDR6", "PCIe 5.0", "White Gaming OC ICE"]
    },
    {
        id: "MOBO-X870-ICE",
        name: "GIGABYTE X870 AORUS ELITE ICE",
        category: "MOBO",
        wattage: 60, // Chipset + VRM overhead
        price: 289.99,
        brand: "GIGABYTE",
        specs: ["AM5 LGA 1718", "Wi-Fi 7 / USB4", "White PCB"]
    },
    {
        id: "RAM-TCREATE-32",
        name: "Team Group T-CREATE Expert",
        category: "RAM",
        wattage: 7, // ~3.5W per stick @ 6000MHz
        price: 104.99,
        brand: "Team Group",
        specs: ["32GB (2x16GB)", "DDR5 6000MHz", "White Heatspreader"]
    },

    // --- POWER & COOLING ---
    {
        id: "PSU-EDGE-1000",
        name: "Lian Li Edge 1000W White",
        category: "POWER",
        wattage: 0, 
        capacity: 1000, 
        price: 209.99,
        brand: "Lian Li",
        specs: ["ATX 3.1 / Gold", "L-Shape Design", "USB Hub Included"]
    },
    {
        id: "AIO-HYDRO-II",
        name: "Lian Li Hydroshift II-S 360",
        category: "COOLING",
        wattage: 25, // Pump + 3.4" IPS Screen
        price: 199.99,
        brand: "Lian Li",
        specs: ["3.4\" IPS LCD Square Screen", "Daisy-Chain", "White"]
    },

    // --- CASE & FANS (Detailed) ---
    {
        id: "CASE-Y70-W",
        name: "HYTE Y70 Touch Infinite",
        category: "CASE",
        wattage: 15, // Integrated 2.5K LCD Power draw
        price: 359.99,
        brand: "HYTE",
        specs: ["2.5K LCD Touchscreen", "Snow White", "Panoramic Glass"]
    },
    {
        id: "FAN-SL-LCD-3P",
        name: "UNI Fan SL LCD Wireless (Reverse)",
        category: "FANS",
        wattage: 9, // ~3W per fan (Motor + LCD + RGB)
        price: 149.99,
        brand: "Lian Li",
        specs: ["1.6\" LCD Display", "Reverse Blade", "Triple Pack"]
    },
    {
        id: "FAN-SL-WL-3P",
        name: "UNI Fan SL Wireless (3-Pack)",
        category: "FANS",
        wattage: 6, // ~2W per fan
        price: 99.99,
        brand: "Lian Li",
        specs: ["2.4 GHz Wireless", "Triple Pack", "White"]
    },
    {
        id: "FAN-SL-WL-1P",
        name: "UNI Fan SL Wireless (Single)",
        category: "FANS",
        wattage: 2, 
        price: 29.99,
        brand: "Lian Li",
        specs: ["Daisy-Chain Design", "Single Pack", "White"]
    },
    {
        id: "FAN-CL120-3P",
        name: "UNI Fan CL120 ARGB (3-Pack)",
        category: "FANS",
        wattage: 5, 
        price: 89.99,
        brand: "Lian Li",
        specs: ["Dual Light Zones", "FDB Bearing", "White"]
    },

    // --- STORAGE ---
    {
        id: "SSD-T705-1TB",
        name: "Crucial T705 1TB Gen5",
        category: "STORAGE",
        wattage: 12, // Gen5 High-load peak
        price: 189.99,
        brand: "Crucial",
        specs: ["13,600 MB/s", "PCIe Gen5 NVMe"]
    },
    {
        id: "SSD-SN7100-1TB",
        name: "WD_BLACK SN7100 1TB",
        category: "STORAGE",
        wattage: 5, // Gen4 average load
        price: 89.99,
        brand: "Western Digital",
        specs: ["PCIe Gen 4.0 x4", "M.2 2280"]
    },

    // --- PERIPHERALS ---
    {
        id: "MONITOR-MSI-4K",
        name: "MSI MAG 274URFW 27” 4K",
        category: "PERIPHERALS",
        wattage: 26, // Average operational draw
        price: 499.99,
        brand: "MSI",
        specs: ["160Hz / 0.5ms GTG", "4K Rapid IPS", "White"]
    },
    {
        id: "KBD-RT80-W",
        name: "YUNZII RT80 Magnetic",
        category: "PERIPHERALS",
        wattage: 2, 
        price: 129.99,
        brand: "YUNZII",
        specs: ["Hall Effect", "8K Polling", "White Gateron"]
    },
    {
        id: "MOUSE-X11-W",
        name: "Attack Shark X11 Superlight",
        category: "PERIPHERALS",
        wattage: 1, 
        price: 59.99,
        brand: "MAGIC-REFINER",
        specs: ["59g Superlight", "22,000 DPI", "White"]
    },
    {
        id: "MOUNT-HYDRA-W",
        name: "Amer Mounts Dual Hydra Arm",
        category: "PERIPHERALS",
        wattage: 0, 
        price: 129.99,
        brand: "Amer Mounts",
        specs: ["Heavy Duty Spring", "12kg Max", "White"]
    }
];